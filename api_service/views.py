# -*- coding: utf-8 -*-
"""
ForgeForth Africa - API Service Views
=====================================
Handles API Service user management and access request operations.
"""
import logging
import secrets
...
def log_audit(user_id, username, action, resource_type, resource_id=None, details=None, request=None):
    ...
    try:
        APIServiceAuditLog.objects.create(
            user_id=user_id,
            username=username,
            action=action,
...
def api_service_login(request):
    """Authenticate an API Service user."""
    try:
        data = request.data
        ...
        try:
            user = APIServiceUser.objects.get(
        ...
        if user.status != APIServiceUser.Status.ACTIVE:
            return Response({
                'success': False,
        ...
        redirect_url = '/docs/'  # Default for API Service
        if user.role == 'admin':
...
def change_password(request):
    """Change password for an API Service user (especially for default password change)."""
    try:
        data = request.data
        username = data.get('username', '').lower().strip()
        ...
        logger.info(f"API Service password changed: {user.username}")
...
@api_view(['POST'])
@permission_classes([AllowAny])
def api_service_logout(request):
    """Logout an API Service user."""
...
def create_access_request(request):
    """Create a new API Service access request."""
    try:
        data = request.data
        ...
        existing = APIServiceAccessRequest.objects.filter(
            email=email,
            status=APIServiceAccessRequest.Status.PENDING
        ).exists()
        ...
        if APIServiceUser.objects.filter(email=email).exists():
            return Response({
                'success': False,
        ...
        access_request = APIServiceAccessRequest.objects.create(
        ...
        logger.info(f"API Service access request created: {access_request.email}")
...
def list_access_requests(request):
    """List all API Service access requests with pagination and filtering."""
    try:
        queryset = APIServiceAccessRequest.objects.all()
...
def get_access_request(request, request_id):
    """Get a specific API Service access request."""
    try:
        access_request = APIServiceAccessRequest.objects.get(id=request_id)
    ...
    except APIServiceAccessRequest.DoesNotExist:
        return Response({
            'success': False,
...
def approve_access_request(request, request_id):
    """Approve an API Service access request and create user account."""
    try:
        access_request = APIServiceAccessRequest.objects.get(id=request_id)

        if access_request.status != APIServiceAccessRequest.Status.PENDING:
            return Response({
                'success': False,
        ...
        # Check if user already exists
        if APIServiceUser.objects.filter(email=username).exists():
            return Response({
                'success': False,
                'error': 'An account with this email already exists'
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        user = APIServiceUser.objects.create_user(
            username=username,
            email=access_request.email,
        ...
        data = request.data
        access_request.status = APIServiceAccessRequest.Status.APPROVED
        ...
        logger.info(f"API Service access request approved: {access_request.email}")
        ...
        try:
            ...
            email_result = send_access_request_approved(
                ...
                login_url='http://localhost:9001/docs/'
            )
            notification_sent = email_result.get('success', False)
...
def reject_access_request(request, request_id):
    """Reject an API Service access request."""
    try:
        access_request = APIServiceAccessRequest.objects.get(id=request_id)
        ...
        logger.info(f"API Service access request rejected: {access_request.email}")
...
def list_users(request):
    ...
    try:
        queryset = APIServiceUser.objects.all()
...
def get_user(request, user_id):
    """Get a specific API Service user."""
    try:
